    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            ...
            for j in range(i-window_size, i):
                if j >= 0:
                    pre_context_words.extend(sentence[j])
                else :
                    pre_context_words.extend(start)
            suf_context_words = []
            for j in range(i+1, i+window_size+1):
                if j<len(sentence):
                    suf_context_words.extend(sentence[j])
                else:
                    suf_context_words.extend(end)
            windowed_data.append((pre_context_words+center_word+suf_context_words,labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings_matrix = tf.Variable(self.pretrained_embeddings, name='embedding')    
    ...
    def add_prediction_op(self):
        ...
        with tf.name_scope("hidden1"):
            h = tf.layers.dense(x, self.config.hidden_size,activation=tf.nn.relu,kernel_initializer=xavier_init)
            h_drop = tf.nn.dropout(h, keep_prob=dropout_rate)

        with tf.name_scope("output"):
            pred = tf.layers.dense(h_drop, self.config.n_classes,kernel_initializer=xavier_init)
        ### END YOUR CODE
    ...
    def add_loss_op(self, pred):
        ...
        with tf.name_scope('loss'):
            xentropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
