...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_matrix = tf.Variable(self.pretrained_embeddings,name = 'embedding')
        # embedding_matrix:embedding矩阵
        # self.inputs_batch:index
        # 该函数避免了矩阵乘法，可以加速
        embeddings = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
        # n_features*embed_size:把n_features个word拼接成了更长的向量
    ...
    def add_prediction_op(self):
        ...
        with tf.name_scope("hidden1"):
            init_xavier = xavier_weight_init()
            W = tf.Variable(init_xavier(shape=[self.config.n_features*self.config.embed_size,self.config.hidden_size]), name='W')
        ...
        with tf.name_scope("output"):
            U = tf.Variable(init_xavier(shape=[self.config.hidden_size,self.config.n_classes]), name='U')
    ...
    def add_loss_op(self, pred):
        ...
        with tf.name_scope('loss'):
            xentropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
